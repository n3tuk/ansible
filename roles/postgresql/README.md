...
| `postgresql_tls_enabled`                | `false`                                | (**optional**) Whether to enable TLS for PostgreSQL connections, using an ACME generated certificate for this server.                                                                             |
| `postgresql_tls_bootstrap`              | `false`                                | (**optional**) Whether to bootstrap TLS by generating the Lego configuration for PostgreSQL to manage the ACME-sourced certificate, but do not configure PostgreSQL to terminate SSL connections. |
| `postgresql_tls_ca_crt`                 | `"/etc/ssl/certs/ca-certificates.crt"` | (**optional**) The CA certificate to use for verifying client certificates when TLS is enabled.                                                                                                   |
